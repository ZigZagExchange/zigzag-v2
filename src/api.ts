...
import { EVMOrderSchema } from 'src/schemas'
...
export default class API extends EventEmitter {
  ...
  start = async (port: number) => {
    ...
    this.VALID_CHAINS.forEach((chainId) => {
      try {
        ...
        const indexB = this.VALID_CHAINS.indexOf(chainId)
        this.VALID_CHAINS.splice(indexB, 1)
      }
    })
  ...
  processOrderEVM = async (
    ...
    if (!this.VALID_CHAINS.includes(chainId))
      throw new Error(
        `ChainId ${chainId} is not valid, only ${this.VALID_CHAINS}`
      )
  ...
  genquote = async (
    ...
    if (baseQuantity) {
      ...
      if (side === 'b') {
        ...
        hardQuoteQuantity = baseQuantity * ladderPrice
      ...
      if (side === 'b') {
        ...
        hardBaseQuantity = quoteQuantity / ladderPrice
        hardPrice = hardQuoteQuantity / hardBaseQuantity
        softPrice = hardPrice * 1.001
      } else {
